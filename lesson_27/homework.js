...
let arrayGuess = [];
for (i = 0; i < 101; i++) { arrayGuess.push(i) }; // [0, 1, 2, 3, 4, ..., 100]
let mid = Math.floor((arrayGuess.length) / 2);

while (arrayGuess.length > 0) {

    if (targetNumber === arrayGuess[mid]) {
        console.log(`Your number is ${arrayGuess[mid]}`)
        break;
    } else if (targetNumber < arrayGuess[mid]) {
        arrayGuess = arrayGuess.slice(0, mid);
        mid = Math.floor((arrayGuess.length) / 2);
    } else if (targetNumber > arrayGuess[mid]) {
        arrayGuess = arrayGuess.slice(mid, arrayGuess.length);
        mid = Math.floor((arrayGuess.length) / 2);
    }
}